...
export const ContextMenu: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const menuItems = [
    ...
    { label: 'Feed: Leaf', icon: Cookie, action: () => { window.dispatchEvent(new CustomEvent('snail:feed', { detail: { foodType: 'leaf' } })); onClose(); } },
    { label: 'Feed: Carrot', icon: Cookie, action: () => { window.dispatchEvent(new CustomEvent('snail:feed', { detail: { foodType: 'carrot' } })); onClose(); } },
    { label: 'Feed: Fruit', icon: Cookie, action: () => { window.dispatchEvent(new CustomEvent('snail:feed', { detail: { foodType: 'fruit' } })); onClose(); } },
    { label: 'Feed: Mushroom', icon: Cookie, action: () => { window.dispatchEvent(new CustomEvent('snail:feed', { detail: { foodType: 'mushroom' } })); onClose(); } },
    { label: 'Feed: Flower', icon: Cookie, action: () => { window.dispatchEvent(new CustomEvent('snail:feed', { detail: { foodType: 'flower' } })); onClose(); } },
